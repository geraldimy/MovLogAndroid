...
public class TopRated extends Fragment{
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        MovieView = inflater.inflate(R.layout.fragment_top_rated, container, false);
    ...
    public void firebaseSearch(String searchText){
        ...
        FirebaseRecyclerAdapter<Model, TopRated.MovieViewHolder> firebaseRecyclerAdapter =
                new FirebaseRecyclerAdapter<Model, MovieViewHolder>(option) {
                    @Override
                    protected void onBindViewHolder(@NonNull TopRated.MovieViewHolder holder, final int position, @NonNull Model model) {
                    ...
                    @NonNull
                    @Override
                    public TopRated.MovieViewHolder onCreateViewHolder(@NonNull ViewGroup viewGroup, int i) {
                        View view = LayoutInflater.from(viewGroup.getContext()).inflate(R.layout.movielist, viewGroup, false);
                        TopRated.MovieViewHolder viewHolder = new TopRated.MovieViewHolder(view);
                        return viewHolder;                    }
                };
    ...
    public void onStart() {
        ...
        FirebaseRecyclerAdapter<Model, TopRated.MovieViewHolder> adapter
                = new FirebaseRecyclerAdapter<Model, TopRated.MovieViewHolder>(options) {

            @Override
            protected void onBindViewHolder(@NonNull final TopRated.MovieViewHolder holder, final int position, @NonNull Model model)
            {
                String title = model.getTitle();
                String genre = model.getGenre();
                String image = model.getImage();
    ...
    private void showSortDialog() {
        String[] sortOptions = {"Highest","Lowest/Not Rated"};
                ...
                .setItems(sortOptions, new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialog, int which) {
                        if (which==0)
                        {
                            ...
                            getFragmentManager()
                                    .beginTransaction()
                                    .detach(TopRated.this)
                                    .attach(TopRated.this)
                                    .commit();
                        }
                        else if (which==1)
                        {
